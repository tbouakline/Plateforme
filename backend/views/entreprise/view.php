...
<div class="entreprise-view w-100">
    ...
    <p>
        <?php if($user->status==9) { ?>
            <?= Html::a('<i class="fas fa-check"></i> Valider', ['valider-compte', 'id' => $user->id, 'status' => 10], ['class' => 'btn btn btn-success shadow px-5']) ?>

            <?= Html::a('<i class="fas fa-window-close"></i> Rejeter', ['valider-compte', 'id' => $user->id, 'status' => 12], ['class' => 'btn btn btn-danger shadow px-5']) ?>
        <?php } ?>
