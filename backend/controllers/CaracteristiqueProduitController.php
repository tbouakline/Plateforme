...
class CaracteristiqueProduitController extends Controller
{
    ...
    public function behaviors()
    {
        return [
            'access' => [
                ...
                'only' => ['index', 'view', 'create', 'update', 'delete', 'validation-caracteristique', 'valider-caracteristique',],
                'rules' => [
                        [
                        ...
                        [
                            'actions' => ['validation-caracteristique',],
                            'roles' => ['@'],
                            'allow' => true,
                        ],
                        [
                            'actions' => ['valider-caracteristique',],
                            'roles' => ['@'],
                            'allow' => true,
                            'matchCallback' => function() {
                                                    $carac=CaracteristiqueProduit::findOne($_GET['id']);
                                                    return (
                                                    ($carac != null)&&(($_GET['status']==10)||($_GET['status']==12))
                                                    );
                                                }
                        ],
                    ],
    ...
    public function actionValidationCaracteristique()
    {
        $requete="Select".
        " id_prod,".
        " designation,".
        ...
        " fini_matiere".
        " From produit".
        " Where id_prod in (Select Distinct id_prod From caracteristique_produit Where status=0)";

        $listeProduits=Yii::$app->db->createCommand($requete)->queryAll();

        return $this->render('validation-caracteristique', [
            'listeProduits' => $listeProduits,
        ]);
    }

    public function actionValiderCaracteristique($id, $status)
    {
        $caracteristique= $this->findModel($id);

        $caracteristique->status=$status;
        $caracteristique->date_validation=date('Y-m-d H:i:s');
        $caracteristique->id_user_validation=Yii::$app->user->identity->id;

        $caracteristique->save();

        return $this->redirect(['validation-caracteristique',]);
    }
